...
tapers samples toward zero at the edges. This reduces the **spectral smearing** caused by abruptly truncating a signal as if it were periodic. Without tapering, even a pure tone leaks when its frequency falls between DFT bins ([DFT, FFT, and Spectral Analysis](#ch-06-dft-fft)).
...
Sidelobes fall faster (~−32 dB first sidelobe region) at the cost of a **wider main lobe** (~2× rectangular). Very common in audio STFT ([STFT, Spectrograms, and Time–Frequency Analysis](#ch-08-stft)).
...
**Snare transient:** long windows blur attack time; short windows widen frequency peaks— motivates [STFT, Spectrograms, and Time–Frequency Analysis](#ch-08-stft).
